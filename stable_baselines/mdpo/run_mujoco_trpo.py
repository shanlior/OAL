...
from stable_baselines.trpo import TRPO
...
def train(env_id, num_timesteps, seed, lam, sgd_steps, klcoeff, log):
    ...
    with tf_util.single_threaded_session():
        ...
        log_path = './experiments/'+str(env_id)+'./OURS-LOADED/noent_klcoeffanneal_samesgdsteps'+str(sgd_steps)+'_longer_wgae0.95_exp1_2_'+str(seed)
        ...
        model = TRPO(MlpPolicy, env, timesteps_per_batch=2048, max_kl=0.01, cg_iters=10, cg_damping=0.1, entcoeff=0.0,
                     gamma=0.99, lam=0.95, vf_iters=5, vf_stepsize=1e-3, verbose=1, seed=seed, sgd_steps=sgd_steps, klcoeff=klcoeff, method="multistep-SGD")
