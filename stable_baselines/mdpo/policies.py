...
from stable_baselines.mdpo.tf_tsallis_statistics import *
...
def apply_squashing_func(mu_, pi_, logp_pi, pi_unsquashed=None):
    """
    ...
    if pi_unsquashed is not None:
        #logp_pi -= tf.reduce_sum(tf.log(1 - pi_ ** 2 + EPS), axis=1)
        return mu_, pi_, logp_pi
    else:
        logp_pi -= tf.reduce_sum(tf.log(1 - policy ** 2 + EPS), axis=1)
        return deterministic_policy, policy, logp_pi


class MDPOOFFPolicy(BasePolicy):
    """
    ...
    def __init__(self, sess, ob_space, ac_space, n_env=1, n_steps=1, n_batch=None, reuse=False, scale=False):
        super(MDPOOFFPolicy, self).__init__(sess, ob_space, ac_space, n_env, n_steps, n_batch, reuse=reuse, scale=scale)
...
class FeedForwardPolicy(MDPOOFFPolicy):
    """
    ...
    def make_actor(self, obs=None, action=None, action_unsquashed=None, reuse=False, scope="pi", log_type="q-log", q_prime=1, reparameterize=True):
        ...
        if log_type=="q-log":
            ...
            if action is None:
                ...
                logp_pi = -0.5 * (((action-mu_)/(tf.exp(log_std)+EPS))**2 + 2*log_std + np.log(2*np.pi)) - tf.log(clip_but_pass_gradient(1 - squashed_pi**2, lower=0, upper=1)+1e-6)
            q_logp_pi = tf.cond(tf.constant(q > 0., dtype=tf.bool), true_fn=lambda: tf.reduce_sum(tf_log_q(tf.exp(logp_pi),q),axis=1), false_fn=lambda: tf.reduce_sum(tf_log_q(tf.minimum(tf.exp(logp_pi),tf.pow(10.,8/(1-q))),q), axis=1))
            #This optimon is for q_prime < 2 case.

        elif log_type=="log":
            print("entering there")
            #if not reparameterize:
            #    pi_ = tf.stop_gradient(pi_)
            #squashed_mu = tf.tanh(mu_)
            #squashed_pi = tf.tanh(pi_)
            #if action is None:
            #    q_logp_pi = gaussian_likelihood(pi_, mu_, log_std) - tf.reduce_sum(tf.log(clip_but_pass_gradient(1 - squashed_pi**2, lower=0, upper=1) + 1e-6), axis=1)
            #else:
            #    q_logp_pi = gaussian_likelihood(action, mu_, log_std) - tf.reduce_sum(tf.log(clip_but_pass_gradient(1 - squashed_pi**2, lower=0, upper=1) + 1e-6), axis=1)
            ...
            if action is None:
                ...
                #assert action_unsquashed is not None
                pi_ = action
                q_logp_pi = gaussian_likelihood(action, mu_, log_std)
                squashed_mu, squashed_pi, q_logp_pi = apply_squashing_func(mu_, action, q_logp_pi)#, action_unsquashed)
        ...
        #else:
        #    logp_pi = gaussian_likelihood(action, mu_, log_std)
